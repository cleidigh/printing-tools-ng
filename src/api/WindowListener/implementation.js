...
var WindowListener = class extends ExtensionCommon.ExtensionAPI {
  ...
  // cleidigh
  // Add pref entry to 78 on tools menu
  add78LegacyPrefsEntry(window, self) {
  ...
  add68PrefsEntry(event) {
    ...
    // noPrefsElem.setAttribute("collapsed", "true"); does not work
                ...
                if (window.location.href == "chrome://messenger/content/messenger.xul" ||
                  ...
                  if (self.pathToOptionsPage) {
                    if (self.getThunderbirdMajorVersion() < 78) {
                      ...
                      // cleidigh - add to tools menu until we transition users to add-ons page
                      self.add78LegacyPrefsEntry(window, self);
