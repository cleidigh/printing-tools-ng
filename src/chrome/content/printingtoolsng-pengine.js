...
var printingtools = {
	...
	reformatLayout: async function () {
		...
		// Create our own styleSheet, much safer
		printingtools.createPTNGStyleSheet();
		...
		if (printingtools.prefs.getBoolPref("extensions.printingtoolsng.messages.style")) {
			...
			printingtools.msgRestoration.ptngStyleSheet.insertRule(rule);
	...
	createPTNGStyleSheet: function () {
		var styleSheet = printingtools.doc.createElement("style");
		styleSheet.setAttribute("id", "ptngStyleSheet");
		printingtools.doc.head.appendChild(styleSheet);
		printingtools.msgRestoration.ptngStyleSheet = styleSheet.sheet;
	},

	removePTNGStyleSheet: function () {
		try {
			if (printingtools.msgRestoration.ptngStyleSheet) {
				printingtools.doc.getElementById("ptngStyleSheet").remove();
			}
		} catch (ex) {
		}
	},
	...
	formatDate: function (msecs, longFormat) {
		...
		try {
			...
			if (longFormat === 0) {
				...
				console.debug('Current date:', testDate2.toLocaleString());
				console.debug('Custom Date Format:', dateformat2);
				console.debug('Formatted Date:', strftime.strftime(dateformat2, testDate2, locale2));
				console.debug('Date Format %H:%M %t:', strftime.strftime("%H:%M %t", testDate2, locale2));
				console.debug('Date Format %t:', strftime.strftime("%t", testDate2, locale2));
				console.debug('Date Format %t %9:', strftime.strftime("%t %9", testDate2, locale2));
				...
				console.log("custom format:", customDateFormat)
