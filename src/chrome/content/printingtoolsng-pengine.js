				...
				if (printingtools.num == 1 && options.printSilent == false && !autoPDFSave) {
					...
					if (url !== "undefinedURL") {
						...
						var l = w.addEventListener("focus", function (e) {
							...
							var mainTableHdrs = printingtools.previewDoc.querySelectorAll(".moz-main-header")
							for (let index = 0; index < mainTableHdrs.length; index++) {
								let element = mainTableHdrs[index];
